...
describe "Persistence Proxy Integration" do

  before(:each) do
    Ripple.load_configuration('spec/proxy.yml', [:test])
    Thread.current[:persistence_proxy] = true
  ...
  it "uses the application/x-snappy content type to serialize the data" do
    expect(Ripple.client).to be_kind_of(PersistenceProxy::Client)
    expect(Ripple.robject_class).to be(PersistenceProxy::Object)
